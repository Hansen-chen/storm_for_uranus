...
RUN /storm/apache-maven-3.6.3/bin/mvn clean package install -DskipTests=true -Dcheckstyle.skip && cd storm-dist/binary && /storm/apache-maven-3.6.3/bin/mvn package -Dgpg.skip=true && cp ./final-package/target/apache-storm-2.2.0-SNAPSHOT.tar.gz /storm/compiled && cd /storm/compiled && tar zxvf apache-storm-2.2.0-SNAPSHOT.tar.gz
